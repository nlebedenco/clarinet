...
template<typename ElementType>
ElementType* begin(Segment<ElementType>& segment) { return segment.begin();  }

template<typename ElementType>
ElementType* end(Segment<ElementType>& segment) { return segment.end(); }

template<typename ElementType = uint8_t>
Segment<ElementType> Memory(void* buf, size_t len) { return Segment<ElementType>((ElementType*)buf, len); }
...
template<typename Range>
auto Equals(const Range& range) -> EqualsRangeMatcher<Range> { return EqualsRangeMatcher<Range>{range}; }

static inline auto Equals(const char* range) -> StringEqualsMatcher { return Equals(std::string(range)); }
