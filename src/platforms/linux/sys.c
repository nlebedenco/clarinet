...
clarinet_error_from_errno(const int err)
{
     ...
     * not be relayed to the end-user because we only operate on non-blocking sockets so all calls to connect() are 
                     ...
                     clarinet_endpoint* restrict src)
{
    ...
    const ssize_t n = recvfrom(sp->handle, buf, len, 0, (struct sockaddr*)&ss, &slen);
    if (n < 0)
        return clarinet_error_from_errno(errno);
