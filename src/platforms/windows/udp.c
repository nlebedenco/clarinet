                  ...
                  uint32_t flags)
{
    ...
    /* The simplest approach is to call WSAStartup every time. According to
    ...
    const DWORD ipv6only = ipv6dual ? off : on;
    const ULONG reuseaddr = (flags & clarinet_so_flag(CLARINET_SO_REUSEADDR)) ? on : off;
    ...
    if (sockfd != INVALID_SOCKET)
    {
         ...
         */
        if ((sendbuf > 0 && setsockopt(sockfd, SOL_SOCKET, SO_SNDBUF, (const char*)&sendbuf, sizeof(sendbuf)) == SOCKET_ERROR)
            || (recvbuf > 0 && setsockopt(sockfd, SOL_SOCKET, SO_RCVBUF, (const char*)&recvbuf, sizeof(recvbuf)) == SOCKET_ERROR)
            /* Reuse address and exclusive address use should always be opposites */
            || setsockopt(sockfd, SOL_SOCKET, SO_REUSEADDR, (const char*)&reuseaddr, sizeof(reuseaddr)) == SOCKET_ERROR
            || setsockoptdep(sockfd, SOL_SOCKET, SO_EXCLUSIVEADDRUSE, (const char*)&exclusiveaddr, sizeof(exclusiveaddr)) == SOCKET_ERROR
            /* setsockopt() does not return an error if setting TTL is not supported, but this is ok here because ttl is considered just a hint not a critical setting */
            || (ss.ss_family == AF_INET && ttl > 0 && setsockopt(sockfd, IPPROTO_IP, IP_TTL, (const char*)&ttl, sizeof(ttl)) == SOCKET_ERROR)
            #if CLARINET_ENABLE_IPV6 && HAVE_SOCKADDR_IN6_SIN6_ADDR
            || (ss.ss_family == AF_INET6 && setsockopt(sockfd, IPPROTO_IPV6, IPV6_V6ONLY, (const char*)&ipv6only, sizeof(ipv6only)) == SOCKET_ERROR)
            || (ss.ss_family == AF_INET6 && ttl > 0 && setsockopt(sockfd, IPPROTO_IPV6, IPV6_UNICAST_HOPS, (const char*)&ttl, sizeof(ttl)) == SOCKET_ERROR) /* curiously IPV6_HOPLIMIT corresponds to IP_RECV_TTL not IP_TTL */
            #endif
            /* Windows 10 doesn't like UDP_CHECKSUM_COVERAGE but it was supposed to support it according to the documentation */
            /* || setsockoptdep(sockfd, IPPROTO_UDP, UDP_CHECKSUM_COVERAGE, (const char*)&on, sizeof(on)) == SOCKET_ERROR */
            /* ensure UDP checksum is not forced to zero, this should be the default though */
            || (ss.ss_family == AF_INET && setsockoptdep(sockfd, IPPROTO_UDP, UDP_NOCHECKSUM, (const char*)&off, sizeof(off)) == SOCKET_ERROR)
