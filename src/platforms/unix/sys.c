                     ...
                     clarinet_endpoint* restrict src)
{
    ...
    /* The POSIX description of recvfrom(2) at https://pubs.opengroup.org/onlinepubs/009696699/functions/recvfrom.html
     * does not document EMSGSIZE errno, and the Linux manual page for recvfrom(2) does not document it either. Linux
    ...
    const ssize_t n = recvmsg(sp->handle, &msg, 0);
    if (n < 0)
        return clarinet_error_from_errno(errno);
    ...
    /* Sanity: improbable but possible if the system disagrees on the size of the sockaddr something is off  */
